...
const WHALE_MODEL_URL = './models/moby-dock.glb';
...
const stepTime = 200; // Miliseconds it takes for the whale to take a step forward, backward, left or right
...
const initaliseValues = () => {
  ...
  whale.position.x = 0;
  whale.position.y = 0;
  whale.rotation.z = WHALE_FORWARD_YAW;
  whale.userData.targetYaw = WHALE_FORWARD_YAW;
...
function Whale() {
  ...
  const bodyColor = 0x2496ED;

  const bodyMat = new THREE.MeshPhongMaterial({ color: bodyColor, flatShading: true });

  const body = new THREE.Mesh(new THREE.BoxGeometry(16*zoom, 12*zoom, 10*zoom), bodyMat);
  body.position.z = 5*zoom;
  body.castShadow = true;
  body.receiveShadow = true;
  whale.add(body);

  const head = new THREE.Mesh(new THREE.BoxGeometry(8*zoom, 10*zoom, 8*zoom), bodyMat);
  head.position.set(10*zoom, 0, 5*zoom);
  head.castShadow = true;
  head.receiveShadow = true;
  whale.add(head);

  const peduncle = new THREE.Mesh(new THREE.BoxGeometry(5*zoom, 8*zoom, 6*zoom), bodyMat);
  peduncle.position.set(-10*zoom, 0, 6*zoom);
  peduncle.castShadow = true;
  peduncle.receiveShadow = true;
  whale.add(peduncle);

  const flukeGeom = new THREE.BoxGeometry(3*zoom, 8*zoom, 2*zoom);
  const flukeLeft = new THREE.Mesh(flukeGeom, bodyMat);
  flukeLeft.position.set(-14*zoom, -6*zoom, 7*zoom);
  flukeLeft.castShadow = true;
  flukeLeft.receiveShadow = true;
  whale.add(flukeLeft);

  const flukeRight = new THREE.Mesh(flukeGeom, bodyMat);
  flukeRight.position.set(-14*zoom, 6*zoom, 7*zoom);
  flukeRight.castShadow = true;
  flukeRight.receiveShadow = true;
  whale.add(flukeRight);

  const fin = new THREE.Mesh(new THREE.BoxGeometry(4*zoom, 3*zoom, 1.5*zoom), bodyMat);
  fin.position.set(3*zoom, -7.5*zoom, 3.5*zoom);
  fin.castShadow = true;
  fin.receiveShadow = true;
  whale.add(fin);

  const eye = new THREE.Mesh(
    new THREE.BoxGeometry(2*zoom, 1*zoom, 2*zoom),
    new THREE.MeshLambertMaterial({ color: 0x111111 })
  );
  eye.position.set(11*zoom, -5.6*zoom, 7*zoom);
  whale.add(eye);

  const containerColors = [0xE04A3E, 0xFFC734, 0x3ECF8E];
  containerColors.forEach((color, i) => {
    const container = new THREE.Mesh(
      new THREE.BoxGeometry(4*zoom, 8*zoom, 4*zoom),
      new THREE.MeshPhongMaterial({ color, flatShading: true })
    );
    container.position.set((-4 + i * 4) * zoom, 0, 12*zoom);
...
function tryLoadWhaleModel(whaleGroup) {
  ...
  loader.load(
    ...
    (gltf) => {
      ...
      model.rotation.set(WHALE_MODEL_ROTATION.x, WHALE_MODEL_ROTATION.y, WHALE_MODEL_ROTATION.z);
      ...
      if (longestAxis > 0) {
        model.scale.setScalar(WHALE_MODEL_TARGET_LENGTH / longestAxis);
      }
      model.updateMatrixWorld(true);
      ...
      while (whaleGroup.children.length > 0) {
        whaleGroup.remove(whaleGroup.children[0]);
      }
      whaleGroup.add(model);

      console.log('[whale] loaded', WHALE_MODEL_URL, {
        rawSize: initialSize,
        appliedScale: model.scale.x,
      });
    },
    undefined,
    (err) => {
      console.info('[whale] no model at ' + WHALE_MODEL_URL + ', keeping primitive whale.', err?.message || err);
    }
  );
}
...
function animate(timestamp) {
  ...
  if (moves[0]) {
    whale.userData.targetYaw = WHALE_FORWARD_YAW + WHALE_YAW_BY_DIRECTION[moves[0]];
  }
  if (whale.userData.targetYaw !== undefined) {
    let angleDelta = whale.userData.targetYaw - whale.rotation.z;
    while (angleDelta > Math.PI) angleDelta -= 2 * Math.PI;
    while (angleDelta < -Math.PI) angleDelta += 2 * Math.PI;
    const easeFactor = 1 - Math.pow(0.5, delta / 40);
    whale.rotation.z += angleDelta * easeFactor;
  }
  ...
  if(stepStartTimestamp) {
    ...
    switch(moves[0]) {
      case 'forward': {
        ...
        whale.position.y = positionY; // initial whale position is 0

        whale.position.z = jumpDeltaDistance;
        break;
      }
      case 'backward': {
        const positionY = currentLane*positionWidth*zoom - moveDeltaDistance
      ...
      case 'left': {
        ...
        whale.position.x = positionX; // initial whale position is 0
        whale.position.z = jumpDeltaDistance;
        break;
      }
      case 'right': {
  ...
  if(lanes[currentLane].type === 'car' || lanes[currentLane].type === 'truck') {
    const whaleMinX = whale.position.x - whaleSize*zoom/2;
    const whaleMaxX = whale.position.x + whaleSize*zoom/2;
